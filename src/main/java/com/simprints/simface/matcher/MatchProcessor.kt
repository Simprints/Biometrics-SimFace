package com.simprints.simface.matcher

class MatchProcessor(): IMatchProcessor {

    override fun matchScore(probe: FloatArray, matchAgainst: FloatArray): Float {
        require(probe.size == matchAgainst.size) { "Arrays must be of the same length" }

        var dotProduct = 0.0
        var magnitude1 = 0.0
        var magnitude2 = 0.0

        for (i in probe.indices) {
            dotProduct += probe[i] * matchAgainst[i]
            magnitude1 += probe[i] * probe[i]
            magnitude2 += matchAgainst[i] * matchAgainst[i]
        }

        magnitude1 = kotlin.math.sqrt(magnitude1)
        magnitude2 = kotlin.math.sqrt(magnitude2)

        // Prevent division by zero
        if (magnitude1 == 0.0 || magnitude2 == 0.0) return 0.0f

        // Return between 0-1
        return 1 - (1 - dotProduct / (magnitude1 * magnitude2)).toFloat()/2
    }

    override fun identificationScore(
        probe: FloatArray,
        matchReferences: List<FloatArray>
    ): Map<FloatArray, Float> {
        return matchReferences.associateWith { matchAgainst ->
            matchScore(probe, matchAgainst)
        }.toList()
            .sortedByDescending { (_, distance) -> distance }
            .toMap()
    }
}