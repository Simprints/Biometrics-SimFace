...
object BlurAnalysis {
     ...
     */
    fun calculateScore(bitmap: Bitmap, minBlur: Double, maxBlur: Double): Double {
        return try {
            val laplacianVariance = calculateLaplacianVariance(bitmap)
            QualityUtils.rampScore(laplacianVariance, minBlur, maxBlur)
        } catch (e: Exception) {
     ...
     */
    private fun calculateLaplacianVariance(bitmap: Bitmap, ksize: Int = 5): Double {
        val mat = Mat()
        val gray = Mat()
